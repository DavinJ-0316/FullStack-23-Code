namespace AfterDelegateEventLINQ.LearningFromMicrosoft
{
		public class TemperatureMonitor : IObservable<Temperature>
		{
				...
				private class Unsubscriber : IDisposable //退订者
				{
						private List<IObserver<Temperature>> _observers; //当前所有订阅者
						private IObserver<Temperature> _observer; // 想退订的订阅者
						...
						public void Dispose() //退订
						{
								if (!(_observer == null)) _observers.Remove(_observer);//退订
						}
				}
				public IDisposable Subscribe(IObserver<Temperature> observer)
				{
						...
						return new Unsubscriber(observers, observer);// 给订阅者一个 退订的 途径/权力
				}
						...
						foreach (var temp in temps)
						{
								...
								if (temp.HasValue)
								{
										if (start || (Math.Abs(temp.Value - previous.Value) >= 0.1m))
										{
												...
												foreach (var observer in observers) observer.OnNext(tempData);
												previous = temp;
								...
								{
										foreach (var observer in observers.ToArray()) if (observer != null) observer.OnCompleted();
